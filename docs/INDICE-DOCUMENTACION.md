...
| ¿Por qué los crons Storytellers fallaban con `ENOENT: /var/task/mailing/app/...` (09–15/05/2026)? | `mailing/STORYTELLERS_MAILS.md` §11 bis — post-mortem completo. Causa: Vercel no empaqueta archivos no-JS en funciones serverless. Solución vigente: funciones TS en `src/lib/storytellers/email-templates.ts` mismo patrón que `getReturnReminderTemplate`. |
| ¿Cómo edito un email Storytellers (05/06/07) y que llegue a producción? | 1) Edita `mailing/app/0X-storytellers-*.html` (espejo visual). 2) `node scripts/sync-storyteller-emails-to-ts.mjs` para regenerar el TS. 3) Commit ambos archivos. Detalle: `mailing/STORYTELLERS_MAILS.md` §3 «Fuente de verdad del HTML en runtime». |
| ¿Cómo reenvío en bloque emails Storytellers que han fallado? | `tsx scripts/storyteller-catch-up-failed.ts --dry-run` (auditar) → `--confirm` (enviar). Deduplica por reserva+tipo, descarta canceladas / sin email / fuera de los 90 días de ventana. CC a `reservas@` por defecto. Detalle: `mailing/STORYTELLERS_MAILS.md` §7.3. |
