...
- **6 imágenes promocionales del email generadas con `gpt-image-2`** (3 hero verticales 4:5 `cover-cta-05/06/07.jpg` + 3 banners horizontales 3:2 `banner-05/06/07.jpg`) usando un único script reutilizable: [`scripts/generate-storytellers-email-promo-images.ts`](./scripts/generate-storytellers-email-promo-images.ts). El modelo recibe la foto base limpia + un brief con texto exacto, paleta `#ea580c / #ffffff / #063971`, tipografía y posición, y compone el cartel completo (jerarquía tipográfica, drop-shadow, gradient para contraste, pill redondeada). NO clicables: el hero invita a deslizar. Se acompañan de CTAs secundarios "outline" (blanco con borde naranja) en el cuerpo + CTA principal naranja sólido al final.
...
- **Render del HTML 05/06/07 en runtime (NUEVO 15/05/2026):** los 3 emails del ciclo viven ahora como **funciones TypeScript** en [`src/lib/storytellers/email-templates.ts`](./src/lib/storytellers/email-templates.ts) (`getStorytellerPickupNightTemplate`, `getStorytellerMidTripTemplate`, `getStorytellerPostTripTemplate`), exactamente igual que `getReturnReminderTemplate` (email 04). Webpack las empaqueta como código JS — Vercel siempre las tiene en `/var/task`. Los HTML de `mailing/app/05–07*.html` se mantienen como espejo visual editable a mano; tras editar uno hay que regenerar el TS con `node scripts/sync-storyteller-emails-to-ts.mjs`. Esto resuelve el incidente `ENOENT` del 09–15/05 (Vercel no empaquetaba archivos no-JS de `mailing/` en la función serverless del cron). Post-mortem completo: [`mailing/STORYTELLERS_MAILS.md`](./mailing/STORYTELLERS_MAILS.md) §11 bis.
- **Catch-up tras incidente:** [`scripts/storyteller-catch-up-failed.ts`](./scripts/storyteller-catch-up-failed.ts) — deduplica los dispatches `failed` del último mes y los reenvía con CC a `reservas@furgocasa.com` respetando idempotencia. Ejecutado el 15/05/2026 para recuperar los 10 envíos perdidos del incidente.
