...
async function main() {
  ...
  const [{ data: bookings }, { data: existingChecklist }, { data: dispatches }] =
    await Promise.all([
      supabase
        .from("bookings")
  ...
  for (const n of notionRows) {
    ...
    if (changed) {
      checklistUpserts++;
      if (samples.length < 8 && n.doc) {
        samples.push(
          `  · ${n.label} → Doc=${n.doc ? "Yes" : "No"} (documentation_received=${merged.documentation_received})`
        );
      }
      if (confirm) {
  ...
  console.log(`📋 Notion Doc=Yes: ${docYes}`);
