...
export default async function LocaleMotorhomeEuropaLatamPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      {/* Section: Special Discount - With blog link */}
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center text-white">
            ...
            <p className="text-lg mb-4 text-orange-100">
              If you're coming from <strong>Australia, USA, UK, Canada, New Zealand, or any other country</strong>, 
              {' '}you get a <strong>-15% discount</strong> on rentals of minimum 2 weeks.
            </p>
            
            {/* Savings examples */}
            <div className="grid md:grid-cols-2 gap-4 mb-8 max-w-2xl mx-auto">
              <div className="bg-white/20 backdrop-blur-sm p-4 rounded-xl">
                <p className="font-bold text-xl mb-1">21 days (3 weeks)</p>
                <p className="text-orange-100">Save up to <strong className="text-white text-2xl">€285</strong></p>
              </div>
              <div className="bg-white/20 backdrop-blur-sm p-4 rounded-xl">
                <p className="font-bold text-xl mb-1">14 days (2 weeks)</p>
                <p className="text-orange-100">Save up to <strong className="text-white text-2xl">€210</strong></p>
              </div>
            </div>
            
            {/* Countries */}
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              {['🇦🇺 Australia', '🇺🇸 USA', '🇬🇧 UK', '🇨🇦 Canada', '🇳🇿 New Zealand', '🇮🇪 Ireland', '🇯🇵 Japan', '🇰🇷 South Korea'].map((country) => (
                <span key={country} className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full text-sm font-medium">
                  {country}
                </span>
              ))}
            </div>
            
            {/* Buttons - Blog first, WhatsApp second */}
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              >
                📋 View full discount conditions
              </LocalizedLink>
              
              <a
                href="https://wa.me/34673414053?text=Hi!%20I'm%20from%20[YOUR%20COUNTRY]%20and%20I%20want%20to%20inquire%20about%20the%20-15%25%20discount"
                target="_blank"
            ...
            <p className="text-sm text-orange-100 mt-6 max-w-2xl mx-auto">
              * Valid for rentals of minimum 2 weeks in Low and Mid Season (September-June). 
              You must prove your international travel with flight tickets.
            </p>
          </div>
        </div>
      </section>
            ...
            {routesArticles.length > 0 ? (
              <>
                {routesArticles.map((article) => (
                  <BlogArticleLink
                    key={article.id}
                    categorySlug={article.category?.slug || 'routes'}
                    slug={article.slug}
