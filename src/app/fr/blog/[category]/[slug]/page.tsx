...
// Locale fijo para esta carpeta /fr/
...
export async function generateStaticParams() {
  const posts = await getAllPublishedPostSlugs('fr');
  console.log(`[generateStaticParams] Pre-generando ${posts.length} posts del blog en francés`);
  return posts;
}
...
export async function generateMetadata({ 
  ...
  if (!post) {
    return {
      ...
      description: "L'article recherché n'existe pas ou a été supprimé."
    };
  }
...
export default async function LocaleBlogPostPage({ 
  ...
  // Obtener post desde el servidor (locale fijo 'fr' para esta carpeta)
  const post = await getPostBySlug(slug, category, locale);
