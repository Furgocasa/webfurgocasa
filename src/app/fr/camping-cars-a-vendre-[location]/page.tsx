...
// Locale: FR (Français) | Kind: sale (Vente)
// ============================================================================
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!location) {
    return { title: t("Emplacement introuvable"), robots: { index: false, follow: false } };
  }

  const title = location.meta_title?.trim() || `${t("Camping-cars à vendre à")} ${location.name}`;
  const description = location.meta_description?.trim() || 
    `${t("Achetez votre camping-car à")} ${location.name}. ${t("Véhicules avec garantie et financement")}.`;
    ...
    <>
      ...
      <section className="relative h-[60vh] min-h-[500px] flex items-center justify-center">
        ...
        <Image
          ...
          alt={location.h1_title || `${t("Camping-cars à vendre à")} ${location.name}`}
          fill
          priority
        ...
        <div className="relative z-20 container mx-auto px-4 text-center text-white">
          <h1 className="text-4xl lg:text-6xl font-heading font-bold mb-6 drop-shadow-lg">
            {location.h1_title || `${t("Camping-cars à vendre à")} ${location.name}`}
          </h1>
          {location.intro_text && (
          ...
          {location.distance_km && (
            <div className="inline-flex items-center gap-2 bg-white/90 backdrop-blur-sm px-6 py-3 rounded-full text-gray-900 font-medium shadow-lg">
              ...
              <span>
                {t("À")} {location.distance_km} km {t("de notre bureau")} ({driveHours}h {t("env.")})
              </span>
            </div>
          )}
        </div>
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-8">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <h2 className="text-2xl font-heading font-bold text-gray-900 mb-2">
                  {t("Camping-cars à vendre à")} {location.name}
                </h2>
                ...
                {location.distance_km && (
                  <p className="text-gray-700">
                    <span className="font-semibold">{t("Distance depuis Murcie")}:</span> {location.distance_km} km 
                    {location.travel_time_minutes && ` (${Math.round(location.travel_time_minutes / 60)}h ${t("env.")})`}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-green-700 bg-green-50 px-4 py-3 rounded-lg">
              ...
              <span className="font-medium">
                {t("Livraison disponible à cet emplacement")}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-3xl lg:text-4xl font-heading font-bold text-gray-900 mb-4">
                {t("Camping-cars à vendre")}
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                {t("Véhicules avec garantie et options de financement")}
              </p>
            </div>
            ...
            <div className="text-center mt-12">
              ...
              >
                {t("Voir tous les véhicules à vendre")}
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl lg:text-4xl font-heading font-bold text-gray-900 mb-4">
              {t("Pourquoi acheter avec Furgocasa?")}
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
            {[
              { icon: Shield, title: t("Garantie incluse"), desc: t("Tous nos véhicules avec garantie") },
              { icon: Euro, title: t("Financement"), desc: t("Options de paiement flexibles") },
              { icon: FileCheck, title: t("Inspection complète"), desc: t("Véhicules inspectés et certifiés") },
              { icon: Phone, title: t("Conseil"), desc: t("Nous vous aidons à choisir") },
            ].map((service, index) => (
      ...
      <section className="py-16 bg-furgocasa-blue text-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl lg:text-4xl font-heading font-bold mb-6">
            {t("Vous cherchez votre camping-car idéal?")}
          </h2>
          <p className="text-xl mb-8 max-w-2xl mx-auto opacity-90">
            {t("Contactez-nous et nous vous aiderons à trouver le véhicule parfait pour vous")}
          </p>
          <LocalizedLink
            href="/ventes"
