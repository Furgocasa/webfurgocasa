...
export async function validateDocImage(params: {
  ...
  const system = [
    ...
    "En `fields` USA SIEMPRE estas claves normalizadas cuando el dato aparezca (además puedes añadir otras):",
    "- full_name: nombre y apellidos completos tal cual aparecen.",
    "- document_number: número de DNI/NIE/pasaporte o número de permiso de conducir.",
    "- birth_date: fecha de nacimiento en formato ISO YYYY-MM-DD.",
    "- expiry_date: fecha de caducidad del documento en formato ISO YYYY-MM-DD (carnet: campo 4b).",
    "- issue_date: fecha de expedición en formato ISO YYYY-MM-DD (carnet: campo 4a).",
    "- license_b_since: SOLO en carnet, fecha (YYYY-MM-DD) de obtención de la categoría B (columna 10 de la fila B). Es la antigüedad del carnet.",
    "- categories: array de categorías del permiso (p.ej. [\"B\",\"A1\"]).",
    "- sex, nationality: si aparecen.",
    "Convierte SIEMPRE las fechas a YYYY-MM-DD (si el documento las muestra como DD MM AA o DD.MM.AAAA, normalízalas).",
